...
class GoogleAnalyticsPhpFilterTest extends WebTestBase {
  ...
  public function testGoogleAnalyticsPhpFilter() {
    ...
    $this->drupalGet('');
    $this->assertRaw('https://www.googletagmanager.com/gtag/js?', '[testGoogleAnalyticsPhpFilter]: Tracking is displayed on frontpage page.');
    $this->drupalGet('admin');
    $this->assertRaw('https://www.googletagmanager.com/gtag/js?', '[testGoogleAnalyticsPhpFilter]: Tracking is displayed on admin page.');

    $this->config('google_analytics.settings')->set('visibility.request_path_pages', '<?php return FALSE; ?>')->save();
    $this->drupalGet('');
    $this->assertNoRaw('https://www.googletagmanager.com/gtag/js?', '[testGoogleAnalyticsPhpFilter]: Tracking is not displayed on frontpage page.');
